    ...
    fun checkNicknameValidate(input : String) : Result<Unit> {
        return when {
            input.length > NICKNAME_MAX_LENGTH -> Result.failure(
                CaramelException(
                    code = UserErrorCode.INVALID_NICKNAME_SIZE,
                    message = "Nicknames must have at least $NICKNAME_MIN_LENGTH character and no more than $NICKNAME_MAX_LENGTH characters.",
                    debugMessage = "닉네임은 $NICKNAME_MIN_LENGTH 자리 이상, $NICKNAME_MAX_LENGTH 자리 이하여야 합니다."
                )
            )
            !input.matches(NICKNAME_VALID_PATTERN) -> Result.failure(
                CaramelException(
